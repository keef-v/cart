...
export default class DisplayCart extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    // Every time cart item is added ir removed , this will handle the calculations of totals
    if (nextProps !== this.props) {
      ...
      this.setState(prevstate =>
        (function() {
          ...
          // calulation stuff is extracted out to seperate function so that you can unit test it
          ...
          if (nextProps.cartItems.length > numItems) {
            ...
            var subTotal =
              parseFloat(prevstate.subTotal, 10) -
              parseFloat(nextProps.amountToMinus, 10);
            parseFloat(prevstate.subTotal, 10) ===
            parseFloat(prevstate.total, 10)
              ? (total = subTotal)
              : (total =
  ...
  applyVoucherDiscount = () => {
    ...
    console.log("vouc----xxx");

    var newTotal = parseFloat(this.state.total, 10) - 10.0;
    newTotal = newTotal.toFixed(2).toString();
    if (newTotal < 0) newTotal = "0.00";
    var newDiscount = parseFloat(this.state.discount, 10) + 10.0;
